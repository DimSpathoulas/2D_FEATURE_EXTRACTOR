...
from tensorflow.keras.utils import get_file  # line 2513
...
class ProposalLayer(KE.Layer):
    ...
    def call(self, inputs):
        ...
        def nms(boxes, scores):
            ...
            # padding = tf.maximum(tf.shape(boxes)[0] - tf.shape(input=proposals)[0], 0)
...
class MaskRCNN(object):
    ...
    def build(self, mode, config):
        ...
        # Inputs
        print('we added projections_rois here')
        projections_rois = KL.Input(shape=[1000, 4], name="projections_rois")
        ...
        if mode == "training":
            ...
            print('we added to fpn_classifier_graph for our projections_rois instead of rpn_rois')

            shared =\
                fpn_classifier_graph(projections_rois, mrcnn_feature_maps, input_image_meta,
                                     config.POOL_SIZE, config.NUM_CLASSES,
                                     train_bn=config.TRAIN_BN,
                                     fc_layers_size=config.FPN_CLASSIF_FC_LAYERS_SIZE)

            model = KM.Model([projections_rois, input_image, input_image_meta, input_anchors],
                             [rpn_rois, rpn_class, rpn_bbox, shared],
                             name='mask_rcnn')
    ...
    def mold_boxes(self, boxes, original_image_shape, image_shape, window):

        boxes = utils.norm_boxes(boxes, original_image_shape[:2])
        ...
        boxes = np.multiply(boxes, scale) + shift

        return boxes


    def detect(self, images, projections, verbose=0):
        ...
        projections = self.mold_boxes(projections, images[0].shape, molded_images[0].shape, windows[0])

        extra_rows = np.full((1000 - projections.shape[0], 4), -1000000)
        projections_rois = np.concatenate((projections, extra_rows), axis=0)
        projections_rois = projections_rois.reshape((1, projections_rois.shape[0], 4))
        ...
        # !!!!!!!!!!!!!!!!!!!!
        unique_rows, unique_indices = np.unique(shared, axis=0, return_index=True)
        #
        # # Print the number of unique rows
        # print("Number of unique rows:", len(unique_rows))
        #
        # # Print the unique rows and their indexes
        # for idx, row in zip(unique_indices, unique_rows):
        #     print("Index:", idx)
        #     print("Unique row:", row)
        # !!!!!!!!!!!!!!!!!!!!

        features = shared[:projections.shape[0]]
